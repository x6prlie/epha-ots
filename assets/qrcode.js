...
QRCodeModel.prototype = {
	...
	createMovieClip: function(target_mc, instance_name, depth) {
		var qr_mc =
			target_mc.createEmptyMovieClip(instance_name, depth);
		var cs = 1;
		this.make();
		for (var row = 0; row < this.modules.length; row++) {
			var y = row * cs;
			for (var col = 0; col < this.modules[row].length;
			     col++) {
				var x = col * cs;
	...
	setupTypeNumber: function(test) {
		...
		for (var i = 0; i < 18; i++) {
			...
			this.modules[Math.floor(
				i / 3)][i % 3 + this.moduleCount - 8 - 3] = mod;
		}
		for (var i = 0; i < 18; i++) {
			var mod = (!test && ((bits >> i) & 1) == 1);
			this.modules[i % 3 + this.moduleCount - 8 -
				     3][Math.floor(i / 3)] = mod;
		}
	},
	setupTypeInfo: function(test, maskPattern) {
		...
		for (var i = 0; i < 15; i++) {
			...
			if (i < 6) {
				...
				this.modules[this.moduleCount - 15 + i][8] =
					mod;
			}
		}
		for (var i = 0; i < 15; i++) {
...
QRCodeModel.createBytes = function(buffer, rsBlocks) {
	...
	for (var r = 0; r < rsBlocks.length; r++) {
		...
		for (var i = 0; i < ecdata[r].length; i++) {
			var modIndex =
				i + modPoly.getLength() - ecdata[r].length;
			ecdata[r][i] = (modIndex >= 0) ? modPoly.get(modIndex) :
							 0;
		}
	}
...
var QRUtil = {
	...
	getLostPoint: function(qrCode) {
		...
		var ratio =
			Math.abs(100 * darkCount / moduleCount / moduleCount -
				 50) /
			5;
		lostPoint += ratio * 10;
		return lostPoint;
	...
	multiply: function(e) {
		...
		for (var i = 0; i < this.getLength(); i++) {
			for (var j = 0; j < e.getLength(); j++) {
				num[i + j] ^=
					QRMath.gexp(QRMath.glog(this.get(i)) +
						    QRMath.glog(e.get(j)));
			}
		}
...
function _getBorderSize(htOption)
{
	var border = Number(htOption.border);
...
var svgDrawer = (function() {
	...
	Drawing.prototype.draw = function(oQRCode) {
		...
		var border = _getBorderSize(_htOption);
		var innerWidth = _htOption.width - border * 2;
		var innerHeight = _htOption.height - border * 2;

		if (innerWidth <= 0 || innerHeight <= 0) {
			throw new Error("QRCode border is too large.");
		}

		var nWidth = innerWidth / nCount;
		var nHeight = innerHeight / nCount;
		...
		var svg = makeSVG("svg", {
			'viewBox': '0 0 ' + String(_htOption.width) + " " +
					   String(_htOption.height),
			'width': '100%',
			'height': '100%',
		...
		svg.appendChild(makeSVG("rect", {
			"fill": _htOption.colorLight,
			"width": String(_htOption.width),
			"height": String(_htOption.height)
		}));
		svg.appendChild(makeSVG("rect", {
			"fill": _htOption.colorDark,
			"width": String(nWidth),
			"height": String(nHeight),
			"id": "template"
		}));
	...
	!_isSupportCanvas() ?
		(function() {
			...
			Drawing.prototype.draw = function(oQRCode) {
				...
				if (innerWidth <= 0 || innerHeight <= 0) {
					throw new Error(
						"QRCode border is too large.");
				}

				var nWidth = innerWidth / nCount;
				var nHeight = innerHeight / nCount;
				_el.style.padding = border + "px";
				...
				var nLeftMarginTable =
					(_htOption.width - border * 2 -
					 elTable.offsetWidth) /
					2;
				var nTopMarginTable =
					(_htOption.height - border * 2 -
					 elTable.offsetHeight) /
					2;
				nLeftMarginTable =
					Math.max(0, nLeftMarginTable);
				nTopMarginTable = Math.max(0, nTopMarginTable);
				elTable.style.margin = nTopMarginTable + "px " +
						       nLeftMarginTable + "px";
			};
			...
			Drawing.prototype.clear = function() {
				this._el.innerHTML = '';
				if (this._elStyleCache) {
					this._el.style.padding =
						this._elStyleCache.padding;
		...
		(function() { // Drawing in Canvas
			...
			if (this._android && this._android <= 2.1) {
				...
				var drawImage = CanvasRenderingContext2D
							.prototype.drawImage;
				CanvasRenderingContext2D.prototype.drawImage =
					function(image, sx, sy, sw, sh, dx, dy,
						 dw, dh) {
					if (("nodeName" in image) &&
		 ...
		 */
			function _safeSetDataURI(fSuccess, fFail)
			{
				...
				if (self._bSupportDataURI === null) {
					...
					return;
				} else if (self._bSupportDataURI === true &&
					   self._fSuccess) {
		 ...
		 */
			Drawing.prototype.draw = function(oQRCode) {
				...
				_oContext.fillRect(0, 0, _htOption.width,
						   _htOption.height);

				for (var row = 0; row < nCount; row++) {
					for (var col = 0; col < nCount; col++) {
						var bIsDark = oQRCode.isDark(
							row, col);
						var nLeft =
							border + col * nWidth;
						var nTop =
							border + row * nHeight;
						_oContext.strokeStyle =
							bIsDark ?
								_htOption
									.colorDark :
								_htOption
...
{
	var replacedText =
		encodeURI(sText).toString().replace(/\%[0-9a-fA-F]{2}/g, 'a');
...
QRCode = function(el, vOption) {
	...
	this._htOption.border = _getBorderSize(this._htOption);
